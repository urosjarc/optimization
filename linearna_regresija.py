...
def narisi_napake():
    ...
    surf = ax.plot_surface(np.array(list(yarr)), np.array(list(karr)), np.array(list(z)), cmap=cm.coolwarm, linewidth=0,
                           antialiased=False)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if napaka < min_napaka:
            ...
            k_naj = k
            y0_naj = y0
            print(min_napaka, hitrost)

        k = k_naj + hitrost * (random.random() - 0.5)
        y0 = y0_naj + hitrost * (random.random() - 0.5)
